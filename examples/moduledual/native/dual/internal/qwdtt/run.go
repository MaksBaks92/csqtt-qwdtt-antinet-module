...
func Run(configContent, resolversPath, profileDir, protectPath string, listenFd int) int {
	...
	// Движок шарит слоты (hashIndex % len(Hashes)). hashes= в ссылке / auto-кеш в
	// MODULE_STATE живут днями; OAuth/calls.start только когда хешей нет вовсе.
	...
	if len(hashes) == 0 {
		...
		log.Printf("[SETTINGS] no hashes in link — auto hashes (cache or %s)", mode)
		emitProgress("%s", "Создаю VK-хеши (Авто API)…")
		got, cleanup, aerr := autoHashesFn(profileDir, protectPath, cfg.ModuleState, dnsPreset, needHashes, mode)
		...
		log.Printf("[SETTINGS] hashes ready: %d (hashMode=%s)", len(hashes), mode)
	} else if len(hashes) < needHashes {
		log.Printf("[SETTINGS] soft-skip top-up: %d hashes present (need=%d) — groups share; no OAuth/calls.start", len(hashes), needHashes)
	}
