...
func newDoHHTTPClient() *http.Client {
	transport := &http2.Transport{
		DialTLSContext: func(ctx context.Context, network, addr string, cfg *tls.Config) (net.Conn, error) {
			...
			// AntiNet: DoH-бутстрап off-TUN, под protect'ом (dial.go::dialWithTimeout — mirror
			// прежнего net.Dialer{Timeout:5s}, он был тут же, в newDoHBootstrapDialer, убранной
			// вместе с этим единственным её caller'ом).
			conn, err := dialWithTimeout(ctx, network, addr, 5*time.Second)
			if err != nil {
				return nil, err
			}
