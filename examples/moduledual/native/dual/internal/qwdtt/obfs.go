...
//
// ─── AntiNet notes on this file ───────────────────────────────────────────────────────────────
//
// This file is taken from upstream VERBATIM and must stay that way. It is not a place where our
// port may "improve" anything: the wire format here is a contract with the author's server, so a
// deviation of a single bit is not a divergence in style but a tunnel that silently drops every
// packet. Anything AntiNet-specific belongs in the transport around it (session.go), never here.
//
// ⚠ An older upstream scheme (`4791f8c`, irregular padding / timing-derived timestamp step /
// random marker bit) was once restored here as a HYPOTHESIS during
// the hysteria2-cascade-behind-qWDTT QUIC handshake investigation, then live-tested: the
// obfuscation scheme made ZERO measurable difference to the target bug — both schemes reproduced
// the failure identically. The real root cause was `helper.go`'s `relayWatchdog` `dev.IpcGet()`
// polling cadence (see that file's `relayWatchdogTick` doc-comment), unrelated to this file.
// Recorded so the same dead end is not re-entered: if a tunnel misbehaves, this file is not the
// suspect. Upstream 1.4.3 has since moved BACK to the regular counter-derived scheme seen below
// (initSeq+count / initTs+count*960, padding always present, no marker bit), which is what the
// current server expects — the 1.3.7 irregular variant is gone from both sides.
